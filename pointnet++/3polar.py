...
def get_model(point_cloud, is_training, num_class, bn_decay=None):
    ...
    x = tf.slice(point_cloud, [0, 0, 0], [-1, -1, 1])
    y = tf.slice(point_cloud, [0, 0, 1], [-1, -1, 1])
    r=tf.sqrt(tf.add(tf.square(x),tf.square(y)))
    theta=tf.atan2(y,x)
    ir = tf.slice(point_cloud, [0,0,2], [-1,-1,2])
    l0_points=tf.concat([r,theta,ir],2)
